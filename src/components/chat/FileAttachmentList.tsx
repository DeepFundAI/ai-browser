...
export const FileAttachmentList: React.FC<FileAttachmentListProps> = ({ files, onFileClick }) => {
  ...
  return (
    <div className="border border-border-message dark:border-border-message-dark rounded-lg bg-tool-call dark:bg-tool-call-dark/30 mb-2">
      {/* Header - clickable to toggle collapse */}
      <div
        className="flex items-center justify-between p-3 cursor-pointer hover:bg-tool-call dark:bg-tool-call-dark/50 transition-colors rounded-lg"
      ...
      {isExpanded && (
        ...
        {filterOptions.map(option => {
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs transition-all ${
                filter === option.key
                  ...
                  : 'bg-tool-call dark:bg-tool-call-dark text-text-12 dark:text-text-12-dark hover:bg-border-message'
              }`}
            >
              {option.label} ({count})
            </button>
          ...
          <div className="space-y-2">
            {filteredFiles.map(file => (
              <div
                ...
                className="flex items-center gap-3 p-2 rounded-md hover:bg-tool-call dark:bg-tool-call-dark cursor-pointer transition-colors group"
              >
                {/* File icon */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-text-01 dark:text-text-01-dark line-clamp-1 group-hover:text-blue-500 transition-colors">
                    {file.name}
                  </div>
                  <div className="text-xs text-text-12 dark:text-text-12-dark flex items-center gap-2">
                ...
                <div className="flex-shrink-0 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <div
                    className="text-text-12 dark:text-text-12-dark hover:text-blue-500 cursor-pointer p-1"
                    onClick={(e) => handleDownload(e, file)}
                    title={t('download')}
                  >
                    <DownloadOutlined />
                  </div>
                  <div className="text-text-12 dark:text-text-12-dark hover:text-blue-500 cursor-pointer p-1" title={t('preview')}>
                    <LinkOutlined />
                  </div>
                </div>
              </div>
          ...
          {filteredFiles.length === 0 && filter !== 'all' && (
            <div className="text-center py-6 text-text-12 dark:text-text-12-dark text-sm">
