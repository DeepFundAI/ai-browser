...
export class PlaybackEngine {
  ...
  private async streamWorkflowMessage(source: DisplayMessage): Promise<void> {
    ...
    if (sourceWorkflow.thought) {
      await this.streamThoughtTextRecursive(sourceWorkflow.thought, 0);
      // Increment completed fragments after thinking is done
      this.state.completedFragments++;
    }

    // Stream agents sequentially using recursion
    if (sourceWorkflow.agents && sourceWorkflow.agents.length > 0) {
      await this.streamAgentsRecursive(sourceWorkflow.agents, 0);
    }
  }
